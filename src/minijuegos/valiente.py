...
def player_timer():
    ...
    if tiempo==0:
        ...
        time.sleep(0.4)
        scream('¡BAM!')
        slow_print('Es un golpe crítico')
        player_time=0
    else:
        print('                                        ')
...
def play(player,tutorial):
    ...
    if not tutorial:
        ...
        p_time=player_time
        if p_time==0:
            slow_print('No presionaste enter a tiempo, hazlo antes para la próxima.')
        else:
            slow_print('Paraste con {:.2f} segundos restantes.'.format(p_time))
    else:
        while p_time==c_time:
            set_player_time()
            time.sleep(1)
